...
namespace PIPT.Agency
{
    public partial class frmDacCatalogue : Form
    {
        ...
        DacProductCategoryCS dacCatalogueCS = new DacProductCategoryCS();
        ...
        private void frmDacAgency_Load(object sender, EventArgs e)
        {
            ...
            dacCatalogueCollection = dacCatalogueCS.GetListCategory();
        ...
        private bool SaveData()
        {
            ...
            if (ucDataButtonStore.DataMode == DataState.Insert)
            {
                ...
                bResult = dacCatalogueCS.Insert(catalogue);
            }
            else
            {
                ...
                bResult = dacCatalogueCS.Update(selectedDacCatalogue);
            }
        ...
        private async void GetAllCatalogue()
        {
            ApiHelper apiHelper = new ApiHelper();
            try
            {
                // Get BindingList DataSource
                dacCatalogueCollection = await apiHelper.GetCatalogueAsync();
                AddObjectIntoBindingList(dacCatalogueCollection);
            }
        ...
        private async void GetAllCatalogue(int Id)
        {
            ApiHelper apiHelper = new ApiHelper();
            try
            {
                // Get BindingList DataSource
                dacCatalogueCollection = await apiHelper.GetCatalogueAsync(Id);
                AddObjectIntoBindingList(dacCatalogueCollection);
            }
            catch (HttpRequestException rex)
            {
        ...
        private void ucDataButtonStore_SaveHandler()
        {
            if (bIsGetDataFromServer)
            {
                ...
                if (dacCatalogueCollection.Count > 0)
                {
                    foreach (DacProductCatalogue Catalogue in dacCatalogueCollection)
                    {
                        bResult = dacCatalogueCS.Insert(Catalogue, Catalogue.Id);
                    }
                }
                if (bResult)
                {
        ...
        private void ucDataButtonStore_DeleteHandler()
        {
            ...
            if (dacCatalogueCS.Delete(selectedDacCatalogue.Code))
            {
        ...
        private void buttonGetCatalogue_Click(object sender, EventArgs e)
        {
            dacCatalogueCollection = dacCatalogueCS.GetListCategory();
            if (dacCatalogueCollection.Count <= 0)
            {
                DialogResult dialogResult = MessageBox.Show("Bạn chưa có nhóm sản phẩm nào! \r\nBạn có muốn nhận nhóm sản phẩm từ server?", this.Text, MessageBoxButtons.YesNo,
