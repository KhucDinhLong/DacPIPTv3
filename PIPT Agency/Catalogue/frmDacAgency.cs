...
namespace PIPT.Agency
{
    public partial class frmDacAgency : Form
    {
        ...
        DacAgency selectedDacAgency = new DacAgency();
        DacAgencyCollection dacAgencyCollection = new DacAgencyCollection();
        DacAgencyCS dacStoreBS = new DacAgencyCS();
        ...
        private void frmDacAgency_Load(object sender, EventArgs e)
        {
            ...
            dacAgencyCollection = dacStoreBS.GetListAgency();
        ...
        private DacAgency GetSelectedStore(string sCode)
        {
        ...
        private bool SaveData()
        {
            ...
            if (ucDataButtonAgency.DataMode == DataState.Insert)
            {
                ...
                bResult = dacStoreBS.Insert(agency);
            }
            else
            {
                ...
                bResult = dacStoreBS.Update(selectedDacAgency);
            }
        ...
        private void ucDataButtonAgency_EditHandler()
        {
            ...
            // Lay du lieu de sua
            selectedDacAgency = GetSelectedStore(gridViewAgency.GetFocusedRowCellValue(gridColumnCode).ToString());
            EnableControls(true);
            textBoxName.Focus();
        }
        ...
        private void ucDataButtonAgency_DeleteHandler()
        {
            ...
            else
            {
                ...
                if (dacStoreBS.Delete(selectedDacAgency.Code))
                {
        ...
        private void CopyToGridControl(string data)
        {
            ...
            agency.OwnerName = rowData[5];
            agency.TaxCode = rowData[6];
            agency.PhoneNum = rowData[7];
            agency.MobileNum = rowData[8];
            agency.Email = rowData[9];
            agency.Region = rowData[10];
            agency.JoinContact = rowData[11];
            agency.DependCode = rowData[12];
            agency.CreatedUser = CommonBS.CurrentUser.LoginID;
            try
            {
        ...
        private void buttonGetAgencyDetail_Click(object sender, EventArgs e)
        {
            if (getCodeTextBox.Text.Trim().Length > 0)
                GetAgency(getCodeTextBox.Text.Trim());
            else
                MessageBox.Show("Bạn chưa nhập mã đại lý để nhận thông tin từ server?", this.Text);
        }

        private async void GetAgency(string Code)
        {
            ApiHelper apiHelper = new ApiHelper();
            // Get BindingList DataSource
            DacAgencyCollection AgencyCollection = await apiHelper.GetAgencyAsync(Code);
            if (AgencyCollection.Count > 0)
            {
                AddObjectIntoBindingList(AgencyCollection);
                ucDataButtonAgency.DataMode = DataState.Insert;
            }
        }
    }
